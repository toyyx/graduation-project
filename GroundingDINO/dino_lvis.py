...
from dataset_dino_lvis import LVISDataset_dino_lvis
...
logger1=init_logger('dino_lvis.log')
...
def init_dataloder(ann_file,image_dir,batch_size,max_data_size=None):
    ...
    dataset = LVISDataset_dino_lvis(ann_file, image_dir, transform=transform, max_data_size=max_data_size)
    dataloder= DataLoader(dataset, batch_size=batch_size, collate_fn=deal_batch_data)
    return dataloder


def update_lvis_data(lvis_data,ann_id,pred_bbox,pred_fix):
    # 更新标注信息
...
def save_lvis_data(lvis_data):
    # 保存新的 COCO 数据集
    new_annotation_path = '/root/autodl-tmp/lvis_v1_val_dino.json'
    with open(new_annotation_path, 'w') as f:
        json.dump(lvis_data, f)

def update_lvis_with_grounding_dino(lvis_annotation_path, image_dir, checkpoint_path):
    # 加载 COCO 数据集
    with open(lvis_annotation_path, 'r') as f:
    ...
    model.to(device)
    dataloder=init_dataloder(lvis_annotation_path,image_dir,1,None)
    num_batch=len(dataloder)
    

    model.eval()
    with torch.no_grad():
        for batch_idx, (samples, w, h, captions, targets) in tqdm(enumerate(dataloder), total=num_batch,desc=f'dino_lvis ==> '):
            ...
            update_lvis_data(lvis_data,targets[0]['id'], [tensor.tolist() for tensor in pred_bbox],pred_fix.tolist() )

        save_lvis_data(lvis_data)


# 示例调用
lvis_annotation_path = '/root/autodl-tmp/lvis_v1_val.json'
image_dir = '/root/autodl-tmp/coco2017'
checkpoint_path= r'./checkpoint/model_epoch_19_lr6.pth'

update_lvis_with_grounding_dino(lvis_annotation_path, image_dir, checkpoint_path)
