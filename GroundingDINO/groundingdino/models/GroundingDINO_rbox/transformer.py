...
class Transformer_rbox(nn.Module):
    def __init__(
        self,
        d_model=256,
        nhead=8,
        ...
        self.enc_out_fix_embed = None
        #self.enc_out_ratio_embed = None
    ...
    #def forward(self, srcs, masks, refpoint_embed, refpoint_fix_embed, refpoint_ratio_embed, pos_embeds, tgt, attn_mask=None, text_dict=None):
    def forward(self, srcs, masks, refpoint_embed, refpoint_fix_embed, pos_embeds, tgt, attn_mask=None, text_dict=None):
        ...
        #input_preparation_start = time.time()
        ...
        #input_preparation_end = time.time()
        #input_preparation_time = input_preparation_end - input_preparation_start
        ...
        #two_stage_processing_start = time.time()
        if self.two_stage_type == "standard":
            #全部候选框（处理0.01-0.09）
            #output_memory, output_proposals, output_fix, output_ratio = gen_encoder_output_proposals( # 生成编码器输出的候选框提议和处理后的特征
                #memory, mask_flatten, spatial_shapes
            #)
            output_memory, output_proposals, output_fix = gen_encoder_output_proposals(
                # 生成编码器输出的候选框提议和处理后的特征
                memory, mask_flatten, spatial_shapes
            )
            ...
            enc_outputs_fix_unselected = (  # 计算未选择的边界框坐标预测结果（加上候选框提议）
                    self.enc_out_fix_embed(output_memory) + output_fix
            )
            #enc_outputs_ratio_unselected = (  # 计算未选择的边界框坐标预测结果（加上候选框提议）
                    #self.enc_out_ratio_embed(output_memory) + output_ratio
            #)
            ...
            refpoint_fix_embed_undetach = torch.gather(  # 收集 topk 个候选框的边界框坐标嵌入（未经过 sigmoid 激活）
                enc_outputs_fix_unselected, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 4)
            )  # unsigmoid
            #refpoint_ratio_embed_undetach = torch.gather(  # 收集 topk 个候选框的边界框坐标嵌入（未经过 sigmoid 激活）
                #enc_outputs_ratio_unselected, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 1)
            #)  # unsigmoid

            # 预测框+候选框 的前topk个，分离梯度
            refpoint_embed_ = refpoint_embed_undetach.detach()  # 分离梯度，得到参考点嵌入
            refpoint_fix_embed_ = refpoint_fix_embed_undetach.detach()  # 分离梯度，得到参考点嵌入
            #refpoint_ratio_embed_ = refpoint_ratio_embed_undetach.detach()  # 分离梯度，得到参考点嵌入
            ...
            if refpoint_embed is not None: # 如果存在之前的参考点嵌入，将其与新的参考点嵌入拼接
                ...
                refpoint_fix_embed = torch.cat([refpoint_fix_embed, refpoint_fix_embed_], dim=1)
                #refpoint_ratio_embed = torch.cat([refpoint_ratio_embed, refpoint_ratio_embed_], dim=1)
                tgt = torch.cat([tgt, tgt_], dim=1)  # 将之前的目标嵌入与新的目标嵌入拼接
            else:
                #refpoint_embed, refpoint_fix_embed, refpoint_ratio_embed, tgt = refpoint_embed_, refpoint_fix_embed_, refpoint_ratio_embed_, tgt_ # 否则，直接使用新的参考点嵌入和目标嵌入
                refpoint_embed, refpoint_fix_embed, tgt = refpoint_embed_, refpoint_fix_embed_, tgt_ # 否则，直接使用新的参考点嵌入和目标嵌入
        ...
        #two_stage_processing_end = time.time()
        #two_stage_processing_time = two_stage_processing_end - two_stage_processing_start
        ...
        #hs, references,references_fix,references_ratio = self.decoder(
        hs, references, references_fix=self.decoder(
            ...
            refpoints_fix_unsigmoid=refpoint_fix_embed.transpose(0, 1),  # refpoint_embed--预测框+候选框 的前topk个
            #refpoints_ratio_unsigmoid=refpoint_ratio_embed.transpose(0, 1),  # refpoint_embed--预测框+候选框 的前topk个
            level_start_index=level_start_index,
            spatial_shapes=spatial_shapes,
            valid_ratios=valid_ratios,
            tgt_mask=attn_mask,
        ...
        #decoder_time = decoder_end - decoder_start
        #########################################################
        # End Decoder
        ...
        """
        print(
            f"Input Preparation Time: {input_preparation_time:.4f}s "
            f"Encoder Time: {encoder_time:.4f}s "
            f"Two Stage Processing Time: {two_stage_processing_time:.4f}s "
            f"Decoder Time: {decoder_time:.4f}s "
            f"Postprocess Time: {postprocess_time:.4f}s "
            f"Total Transformer Time: {total_time:.4f}s "
        )
        """

        #return hs, references,references_fix,references_ratio, hs_enc, ref_enc, init_box_proposal
        return hs, references,references_fix, hs_enc, ref_enc, init_box_proposal
...
class TransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        refpoints_fix_unsigmoid: Optional[Tensor] = None,  # num_queries, bs, 4
        #refpoints_ratio_unsigmoid: Optional[Tensor] = None,  # num_queries, bs, 2
        # for memory
        ...
        reference_points_fix = refpoints_fix_unsigmoid.sigmoid()  # 对未经过 sigmoid 激活的参考点应用 sigmoid 函数，得到参考点
        #reference_points_ratio = refpoints_ratio_unsigmoid.sigmoid()  # 对未经过 sigmoid 激活的参考点应用 sigmoid 函数，得到参考点
        ref_points = [reference_points] # 创建列表用于存储参考点的更新结果 sigmoid
        ref_points_fix = [reference_points_fix]  # 创建列表用于存储参考点的更新结果 sigmoid
        #ref_points_ratio = [reference_points_ratio]  # 创建列表用于存储参考点的更新结果 sigmoid
        ...
        for layer_id, layer in enumerate(self.layers):  # 遍历每一层解码器
            ...
            if self.bbox_embed is not None:
                ...
                reference_fix_before_sigmoid = inverse_sigmoid(reference_points_fix)  # 对参考点应用 inverse_sigmoid 函数，得到 sigmoid 之前的参考点
                #reference_ratio_before_sigmoid = inverse_sigmoid(reference_points_ratio)  # 对参考点应用 inverse_sigmoid 函数，得到 sigmoid 之前的参考点

                ##预测框的修正量
                delta_unsig = self.bbox_embed[layer_id](output)     # 通过 bbox_embed 对输出进行处理，得到偏移量
                delta_fix_unsig = self.fix_embed[layer_id](output)  # 通过 bbox_embed 对输出进行处理，得到偏移量
                #delta_ratio_unsig = self.ratio_embed[layer_id](output)  # 通过 bbox_embed 对输出进行处理，得到偏移量

                outputs_unsig = delta_unsig + reference_before_sigmoid  # 将偏移量与 sigmoid 之前的参考点相加，得到更新后的未经过 sigmoid 激活的参考点
                outputs_fix_unsig = delta_fix_unsig + reference_fix_before_sigmoid  # 将偏移量与 sigmoid 之前的参考点相加，得到更新后的未经过 sigmoid 激活的参考点
                #outputs_ratio_unsig = delta_ratio_unsig + reference_ratio_before_sigmoid  # 将偏移量与 sigmoid 之前的参考点相加，得到更新后的未经过 sigmoid 激活的参考点

                new_reference_points = outputs_unsig.sigmoid() # 对更新后的未经过 sigmoid 激活的参考点应用 sigmoid 函数，得到新的参考点
                new_reference_points_fix = outputs_fix_unsig.sigmoid()  # 对更新后的未经过 sigmoid 激活的参考点应用 sigmoid 函数，得到新的参考点
                #new_reference_points_ratio = outputs_ratio_unsig.sigmoid()  # 对更新后的未经过 sigmoid 激活的参考点应用 sigmoid 函数，得到新的参考点

                reference_points = new_reference_points.detach()  # 将新的参考点从计算图中分离出来，避免梯度传播
                reference_points_fix = new_reference_points_fix.detach()  # 将新的参考点从计算图中分离出来，避免梯度传播
                #reference_points_ratio = new_reference_points_ratio.detach()  # 将新的参考点从计算图中分离出来，避免梯度传播
                # if layer_id != self.num_layers - 1:
                ref_points.append(new_reference_points) # 将新的参考点添加到参考点更新结果列表中
                ref_points_fix.append(new_reference_points_fix)  # 将新的参考点添加到参考点更新结果列表中
                #ref_points_ratio.append(new_reference_points_ratio)  # 将新的参考点添加到参考点更新结果列表中
        ...
        return [  # 对中间输出和参考点更新结果进行维度转置，并返回
            ...
            [itm_refpoint_fix.transpose(0, 1) for itm_refpoint_fix in ref_points_fix],
            #[itm_refpoint_ratio.transpose(0, 1) for itm_refpoint_ratio in ref_points_ratio],
        ]
