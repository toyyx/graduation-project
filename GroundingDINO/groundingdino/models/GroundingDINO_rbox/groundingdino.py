...
class GroundingDINO_rbox(nn.Module):
    ...
    def __init__(
        ...
        text_encoder_type='/root/GroundingDINO/models--bert-base-uncased/snapshots/86b5e0934494bd15c9632b12f734a8a67f723594'
        # bert
        ...
        _fix_embed = MLP(hidden_dim, hidden_dim, 4, 3)  # 初始化边界框嵌入模块，使用 MLP 网络
        #_ratio_embed = MLP(hidden_dim, hidden_dim, 1, 3)  # 初始化边界框嵌入模块，使用 MLP 网络
        ...
        nn.init.constant_(_fix_embed.layers[-1].weight.data, 0)  # 初始化 MLP 最后一层的权重为 0
        nn.init.constant_(_fix_embed.layers[-1].bias.data, 0)  # 初始化 MLP 最后一层的偏置为 0
        #nn.init.constant_(_ratio_embed.layers[-1].weight.data, 0)  # 初始化 MLP 最后一层的权重为 0
        #nn.init.constant_(_ratio_embed.layers[-1].bias.data, 0)  # 初始化 MLP 最后一层的偏置为 0

        if dec_pred_bbox_embed_share:  # 如果解码器预测边界框嵌入共享，使用同一个边界框嵌入模块多次
            box_embed_layerlist = [_bbox_embed for i in range(transformer.num_decoder_layers)]
            fix_embed_layerlist = [_fix_embed for i in range(transformer.num_decoder_layers)]
            #ratio_embed_layerlist = [_ratio_embed for i in range(transformer.num_decoder_layers)]
        else: # 如果不共享，复制多个边界框嵌入模块
            box_embed_layerlist = [
                copy.deepcopy(_bbox_embed) for i in range(transformer.num_decoder_layers)
            ]
            fix_embed_layerlist = [
                copy.deepcopy(_fix_embed) for i in range(transformer.num_decoder_layers)
            ]
            #ratio_embed_layerlist = [
                #copy.deepcopy(_ratio_embed) for i in range(transformer.num_decoder_layers)
            #]
        ...
        self.fix_embed = nn.ModuleList(fix_embed_layerlist)  # 将边界框嵌入模块列表转换为 ModuleList
        #self.ratio_embed = nn.ModuleList(ratio_embed_layerlist)  # 将边界框嵌入模块列表转换为 ModuleList
        ...
        self.transformer.decoder.fix_embed = self.fix_embed  # 将边界框嵌入模块列表赋值给 Transformer 解码器
        #self.transformer.decoder.ratio_embed = self.ratio_embed  # 将边界框嵌入模块列表赋值给 Transformer 解码器
        ...
        if two_stage_type != "no":
            if two_stage_bbox_embed_share: # 如果两阶段边界框嵌入共享，确保解码器预测边界框嵌入也共享
                ...
                self.transformer.enc_out_fix_embed = _fix_embed  # 将同一个边界框嵌入模块赋值给 Transformer 编码器输出
                #self.transformer.enc_out_ratio_embed = _ratio_embed  # 将同一个边界框嵌入模块赋值给 Transformer 编码器输出
            else:
                self.transformer.enc_out_bbox_embed = copy.deepcopy(_bbox_embed)  # 如果不共享，复制一个边界框嵌入模块赋值给 Transformer 编码器输出
                self.transformer.enc_out_fix_embed = copy.deepcopy(_fix_embed)  # 如果不共享，复制一个边界框嵌入模块赋值给 Transformer 编码器输出
                #self.transformer.enc_out_ratio_embed = copy.deepcopy(_ratio_embed)  # 如果不共享，复制一个边界框嵌入模块赋值给 Transformer 编码器输出
    ...
    def forward(self, samples: NestedTensor, targets: List = None, **kw):
        ...
        #text_processing_end = time.time()
        #text_processing_time = text_processing_end - text_processing_start
        ...
        #image_processing_time = image_processing_end - image_processing_start

        #transformer_processing_start = time.time()
        #input_query_bbox =input_query_bbox_fix =input_query_bbox_ratio = input_query_label = attn_mask = dn_meta = None# 初始化输入查询的边界框、标签、注意力掩码和去噪元数据为 None
        input_query_bbox =input_query_bbox_fix = input_query_label = attn_mask = dn_meta = None# 初始化输入查询的边界框、标签、注意力掩码和去噪元数据为 None
        ...
        #hs, reference,reference_fix,reference_ratio, hs_enc, ref_enc, init_box_proposal = self.transformer(
            #srcs, masks, input_query_bbox,input_query_bbox_fix,input_query_bbox_ratio, self.poss, input_query_label, attn_mask, text_dict
        #)
        hs, reference, reference_fix, hs_enc, ref_enc, init_box_proposal = self.transformer(
            srcs, masks, input_query_bbox, input_query_bbox_fix, self.poss, input_query_label, attn_mask, text_dict
        )
        #transformer_processing_end = time.time()
        #transformer_processing_time = transformer_processing_end - transformer_processing_start

        #bbox_update_start = time.time()
        # deformable-detr-like anchor update
        outputs_coord_list = []# 用于存储每层解码器输出的边界框坐标
        outputs_fix_list = []  # 用于存储每层解码器输出的边界框坐标
        #outputs_ratio_list = []  # 用于存储每层解码器输出的边界框坐标
        #for dec_lid, (layer_ref_sig,layer_ref_fix_sig,layer_ref_ratio_sig, layer_bbox_embed,layer_fix_embed,layer_ratio_embed, layer_hs) in enumerate(# 遍历除最后一层外的所有解码器层的参考点、边界框嵌入模块和隐藏状态
            #zip(reference[:-1],reference_fix[:-1],reference_ratio[:-1], self.bbox_embed,self.fix_embed,self.ratio_embed, hs)
        #):
        for dec_lid, (layer_ref_sig, layer_ref_fix_sig, layer_bbox_embed, layer_fix_embed,layer_hs) in enumerate(  # 遍历除最后一层外的所有解码器层的参考点、边界框嵌入模块和隐藏状态
            zip(reference[:-1], reference_fix[:-1],  self.bbox_embed, self.fix_embed, hs)
        ):
            layer_delta_unsig = layer_bbox_embed(layer_hs) # 计算当前层隐藏状态经过边界框嵌入模块后的输出，得到边界框的偏移量
            layer_delta_fix_unsig = layer_fix_embed(layer_hs)  # 计算当前层隐藏状态经过边界框嵌入模块后的输出，得到边界框的偏移量
            #layer_delta_ratio_unsig = layer_ratio_embed(layer_hs)  # 计算当前层隐藏状态经过边界框嵌入模块后的输出，得到边界框的偏移量
            ...
            layer_outputs_fix_unsig = layer_delta_fix_unsig + inverse_sigmoid(layer_ref_fix_sig)  # 将边界框的偏移量与参考点经过逆 sigmoid 函数处理后的值相加，得到未经过 sigmoid 处理的边界框坐标
            #layer_outputs_ratio_unsig = layer_delta_ratio_unsig + inverse_sigmoid(layer_ref_ratio_sig)  # 将边界框的偏移量与参考点经过逆 sigmoid 函数处理后的值相加，得到未经过 sigmoid 处理的边界框坐标

            layer_outputs_sig = layer_outputs_unsig.sigmoid()  # 对未经过 sigmoid 处理的边界框坐标应用 sigmoid 函数，将其值映射到 [0, 1] 区间
            layer_outputs_fix_sig = layer_outputs_fix_unsig.sigmoid()  # 对未经过 sigmoid 处理的边界框坐标应用 sigmoid 函数，将其值映射到 [0, 1] 区间
            #layer_outputs_ratio_sig = layer_outputs_ratio_unsig.sigmoid()  # 对未经过 sigmoid 处理的边界框坐标应用 sigmoid 函数，将其值映射到 [0, 1] 区间

            outputs_coord_list.append(layer_outputs_sig)# 将当前层处理后的边界框坐标添加到输出列表中
            outputs_fix_list.append(layer_outputs_fix_sig)  # 将当前层处理后的边界框坐标添加到输出列表中
            #outputs_ratio_list.append(layer_outputs_ratio_sig)  # 将当前层处理后的边界框坐标添加到输出列表中
        outputs_coord_list = torch.stack(outputs_coord_list)# 将存储每层边界框坐标的列表转换为一个张量
        outputs_fix_list = torch.stack(outputs_fix_list)  # 将存储每层边界框坐标的列表转换为一个张量
        #outputs_ratio_list = torch.stack(outputs_ratio_list)  # 将存储每层边界框坐标的列表转换为一个张量
        #bbox_update_end = time.time()
        #bbox_update_time = bbox_update_end - bbox_update_start

       # class_prediction_start = time.time()
        # output
        # 存储每层解码器输出的类别预测结果
        ...
        #out = {"pred_logits": outputs_class[-1], "pred_boxes": outputs_coord_list[-1],"pred_fix": outputs_fix_list[-1],"pred_ratio": outputs_ratio_list[-1]}
        out = {"pred_logits": outputs_class[-1], "pred_boxes": outputs_coord_list[-1],"pred_fix": outputs_fix_list[-1]}

        #class_prediction_end = time.time()
        #class_prediction_time = class_prediction_end - class_prediction_start
        ...
        """
        print(
            f"Text Processing Time: {text_processing_time:.4f}s "
            f"Image Processing Time: {image_processing_time:.4f}s "
            f"Transformer Processing Time: {transformer_processing_time:.4f}s "
            f"Bbox Update Time: {bbox_update_time:.4f}s "
            f"Class Prediction Time: {class_prediction_time:.4f}s "
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name="groundingdino_rbox")
def build_groundingdino_rbox(args):
