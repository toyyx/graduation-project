...
use_wandb = False
use_amp = False
resume = "./work_dir/MedSAM-ViT-B-20250410-1145/medsam_model_best.pth"  ##checkpoint path
device = "cuda:0"
...
def main():
    ...
    if resume is not None:  # 如果指定了从检查点恢复训练
        if os.path.isfile(resume):
            ...
            print(f'update model by {resume}')
    ...
    print(
        f'Time: {datetime.now().strftime("%Y%m%d-%H%M")}, Loss: {epoch_loss},iou:{total_iou}'
    )
