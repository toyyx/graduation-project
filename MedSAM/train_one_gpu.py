...
for step, (image, gt, bboxes, names_temp) in enumerate(tr_dataloader):# 遍历数据加载器，获取一个批次的数据
    print(image.shape, gt.shape, bboxes.shape) # 打印图像、掩码和边界框的形状
    # show the example
...
parser.add_argument(# 添加训练数据路径的参数
    "-i",
    "--tr_npy_path",
    type=str,
...
if args.use_wandb:# 如果使用wandb监控训练过程
    import wandb
...
model_save_path = join(args.work_dir, args.task_name + "-" + run_id)# 构建模型保存路径
...
def main():
    ...
    start_epoch = 0
    if args.resume is not None:# 如果指定了从检查点恢复训练
