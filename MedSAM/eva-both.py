...
tr_dataset = NpyDataset("/root/autodl-tmp/MedSAM_data/npy_val/CT_Abd")  # 创建训练数据集实例
...
# 直接指定命令行参数
tr_npy_path = "/root/autodl-tmp/MedSAM_data/npy_val/CT_Abd"
...
use_wandb = False
use_amp = False
resume1 = "./work_dir/MedSAM-ViT-B-20250410-1145-bbox/medsam_model_best.pth"  ##checkpoint path
resume2 = "./work_dir/MedSAM-ViT-B-20250411-0759-rbox/medsam_model_best.pth"  ##checkpoint path
...
class MedSAM(nn.Module):  # 自定义MedSAM模型类
    ...
    def inference_bbox(self, image, box):
        image_embedding = self.image_encoder(image)  # (B, 256, 64, 64)
        # 切断 image_embedding 的梯度传播
        image_embedding = image_embedding.detach()
    ...
    def inference_rbox(self, image, box, rbox):
        image_embedding = self.image_encoder(image)  # (B, 256, 64, 64)
        # 切断 image_embedding 的梯度传播
        image_embedding = image_embedding.detach()
...
def main():
    ...
    if log.missing_keys:
        ...
        sam_model.prompt_encoder.rbox_point_embeddings[0].weight.data = checkpoint2[
            'prompt_encoder.rbox_point_embeddings.0.weight']
        sam_model.prompt_encoder.rbox_point_embeddings[1].weight.data = checkpoint2[
            'prompt_encoder.rbox_point_embeddings.1.weight']
        sam_model.prompt_encoder.rbox_point_embeddings[2].weight.data = checkpoint2[
            'prompt_encoder.rbox_point_embeddings.2.weight']
        sam_model.prompt_encoder.rbox_point_embeddings[3].weight.data = checkpoint2[
            'prompt_encoder.rbox_point_embeddings.3.weight']
    else:
        print("没有缺失的键，模型加载完整。")
    ...
    medsam_model1 = copy.deepcopy(medsam_model)
    medsam_model2 = copy.deepcopy(medsam_model)
    print(  # 打印模型的总参数数量
    ...
    if resume1 is not None and resume2 is not None:  # 如果指定了从检查点恢复训练
        if os.path.isfile(resume1):
            ## Map model to be loaded to specified single GPU
            checkpoint = torch.load(resume1, map_location=device)
            medsam_model1.load_state_dict(checkpoint["model"])
            print(f'medsam_model1 update model by {resume1}')
        if os.path.isfile(resume2):
            ## Map model to be loaded to specified single GPU
            checkpoint = torch.load(resume2, map_location=device)
            medsam_model2.load_state_dict(checkpoint["model"])
            print(f'medsam_model2 update model by {resume2}')
    ...
    for step, (image, gt2D, boxes, rboxes, _) in enumerate(tqdm(train_dataloader)):
        ...
        if use_amp:  # 如果使用混合精度训练
            ...
            with torch.autocast(device_type="cuda", dtype=torch.float16):  # 如果使用混合精度训练
                medsam_pred1 = medsam_model1.inference_bbox(image, boxes_np)
                medsam_pred2 = medsam_model2.inference_rbox(image, boxes_np, rboxes_np)
                loss1 = seg_loss(medsam_pred1, gt2D) + ce_loss(
                    medsam_pred1, gt2D.float()
                )
                loss2 = seg_loss(medsam_pred2, gt2D) + ce_loss(
                    medsam_pred2, gt2D.float()
                )
                iou1 = calculate_iou(medsam_pred1.squeeze(1) > 0.0, gt2D)
                iou2 = calculate_iou(medsam_pred2.squeeze(1) > 0.0, gt2D)
            ...
            loss1 = seg_loss(medsam_pred1, gt2D) + ce_loss(medsam_pred1, gt2D.float())
            loss2 = seg_loss(medsam_pred2, gt2D) + ce_loss(medsam_pred2, gt2D.float())
    ...
    epoch_loss1 /= step  # 计算当前轮的平均损失
    epoch_loss2 /= step
    total_iou1 /= step
    total_iou2 /= step

    if use_wandb:
        wandb.log({"epoch_loss1": epoch_loss1})
    print(
        f'Time: {datetime.now().strftime("%Y%m%d-%H%M")}, Loss1: {epoch_loss1}, iou1:{total_iou1}, Loss2: {epoch_loss2}, iou2:{total_iou2}'
    )
