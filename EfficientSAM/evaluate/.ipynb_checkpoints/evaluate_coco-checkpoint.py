        ...
        for image_index, (img_path, annotations, w, h) in tqdm(enumerate(val_image_dataloader, start=1),total=len(val_image_dataloader), desc=f'Evaluate ==> '):
            ...
            for batch_idx, (bbox_tensor, rbox_tensor, gt_mask_tensor) in enumerate(oneImage_dataloader, start=1):#point_tensor:b12    gt_mask_tensor:bhw
                if image_embeddings is None:
                    image_embeddings, input_h, input_w = process_single_image(sam, img_path[0])####
                ...
                total_iou += iou
                print(f'VAL ==> file:{image_index}/{num_val}  batch:{batch_idx}/{len(oneImage_dataloader)}, focal_loss: {focal}, dice_loss: {dice}, final_loss:{loss}, accuracy:{accuracy}, iou:{iou}')
                logging.info(f'VAL ==> file:{image_index}/{num_val}  batch:{batch_idx}/{len(oneImage_dataloader)}, focal_loss: {focal}, dice_loss: {dice}, final_loss:{loss}, accuracy:{accuracy}, iou:{iou}')
