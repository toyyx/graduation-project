...
class MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        # Tile the image embedding for all queries.
        #if image_embeddings.shape[0] == 1 and batch_size != 1:
        image_embeddings_tiled = torch.repeat_interleave(image_embeddings, batch_size, dim=0)  # 将 image_embeddings 在批次维度上重复 tokens 的批次大小次，确保每个掩码都有对应的图像特征
