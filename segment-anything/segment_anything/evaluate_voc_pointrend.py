...
def evaluate(batch_size):
    logging.basicConfig(filename='evaluate_voc_combine.log', level=logging.INFO,
    ...
    with torch.no_grad():
        # 遍历文件夹中的所有文件
        for image_index, (image_path, instance_seg_path, annotation_path) in tqdm(enumerate(path_dataloader, start=1),
                                                                                  total=len(path_dataloader),
                                                                                  desc=f'Evaluate ==> '):
            ...
            for batch_idx, (bbox_tensor, rbox_tensor, gt_mask_tensor) in enumerate(dataloader,
                ...
                low_res_masks = process_single_mask_pointrend(sam, image_embeddings, input_size, original_size,bbox_tensor, rbox_tensor)
