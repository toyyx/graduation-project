...
def sampling_points(pred_mask, N, k=3, beta=0.75, training=True):
    ...
    if not training:
        ...
        #print(f'record 111 :{end_time_temp-start_time_temp}')
        ...
        #end_time_temp = time.time()
        #print(f'record 222 :{end_time_temp-start_time_temp}')
    ...
    # 合并重要性采样点和随机采样点
    #visualize_sampling_edge(image_path,pred_mask,edge_masks)
    #visualize_sampling_points(image_path,pred_mask,combined_indices)
...
class StandardPointHead(nn.Module):
    ...
    def __init__(self):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(fc_dim_in, 2 * fc_dim_in),
            nn.ReLU(),
            nn.Linear(2 * fc_dim_in, 4 * fc_dim_in),
            nn.ReLU(),
            nn.Linear(4 * fc_dim_in, 4 * fc_dim_in),
        )
        """
        ...
        self.norm1 = nn.LayerNorm(4 * fc_dim_in)  # 创建一个层归一化层，用于对 MLP 块的输出进行归一化

        self.dropout = nn.Dropout(p=0.2)
        self.predictor = nn.Sequential(
            nn.Linear(4 * fc_dim_in, 2 * fc_dim_in),  # 预测层
            nn.ReLU(),
            nn.Linear(2 * fc_dim_in, 1 * fc_dim_in),  # 预测层
            nn.ReLU(),
            nn.Linear(fc_dim_in, output_channels)
        )
    ...
    def forward(self, img_samples, img_samples_boundrys,  mask_samples,
                mask_samples_boundrys):

        if isinstance(img_samples, torch.Tensor) and isinstance(mask_samples,
                                                                torch.Tensor):  # 处理 fine_grained_features 和 coarse_features 为张量的情况
            # 处理 fine_grained_features 和 coarse_features 为列表的情况
            output_list = []
            for img_sample, img_samples_boundry,  mask_sample, mask_samples_boundry in zip(
                    img_samples,
...
class PointRend(nn.Module):
    ...
    def forward_train(self, image_path,img_feature, pred_mask, valid_h=1.0, valid_w=1.0):
        points = sampling_points(image_path,pred_mask, (pred_mask.shape[-1] // 64) * (pred_mask.shape[-2] // 64), self.k,
...
def visualize_sampling_points(image_path, pred_mask, combined_indices, save_folder='visualize_sampling_points_new'):
    """
    可视化预测掩码和采样点，并保存为图片
    :param image_path: 原图的路径
    :param pred_mask: 预测掩码，形状为 (b, c, h, w)
    :param combined_indices: 合并后的采样点索引，形状为 (b, N,)
    ...
    for i in range(b):
        ...
        # 读取原图
        image = Image.open(image_path).resize((w_img, h_img))
        plt.imshow(image)
        ...
        # 显示采样点
        indices = combined_indices[i].cpu().numpy()
        h_coords = indices // w_img
        w_coords = indices % w_img
...
def visualize_sampling_edge(image_path, pred_mask, edge_masks, save_folder='visualize_sampling_edge_new'):
    """
    可视化预测掩码、采样点和膨胀后的掩码边缘，并保存为图片
    :param image_path: 原图的路径
    :param pred_mask: 预测掩码，形状为 (b, c, h, w)
    :param combined_indices: 合并后的采样点索引，形状为 (b, N,)
    :param edge_masks: 膨胀后的掩码边缘，形状为 (b, h, w)，二值掩码
    :param save_folder: 保存图片的文件夹路径
    """
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)
    ...
    for i in range(b):
        ...
        # 显示膨胀后的掩码边缘
        edge_mask = edge_masks[i].cpu().numpy()
        plt.imshow(edge_mask, cmap='Reds', alpha=0.5)

        plt.title(f'Predicted Mask and Edge Mask {i + 1}')
...
def edge_detection_mask_old(pred_mask, dilation_kernel_size=3):
    """
...
def edge_detection_mask(pred_mask, dilation_kernel_size=3):
    ...
    # 使用 PyTorch 进行边缘检测
    # 计算梯度
    sobel_x = torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=torch.float32, device=pred_mask.device).unsqueeze(0).unsqueeze(0)
    sobel_y = torch.tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], dtype=torch.float32, device=pred_mask.device).unsqueeze(0).unsqueeze(0)

    grad_x = F.conv2d(pred_mask.unsqueeze(1), sobel_x, padding=1).squeeze(1)
    grad_y = F.conv2d(pred_mask.unsqueeze(1), sobel_y, padding=1).squeeze(1)
    ...
    # 使用 PyTorch 进行膨胀操作
    kernel = torch.ones((1, 1, dilation_kernel_size, dilation_kernel_size), dtype=torch.float32, device=pred_mask.device)
    dilated_edges = F.conv2d(edges.unsqueeze(1), kernel, padding=dilation_kernel_size // 2).squeeze(1)
    dilated_edges = (dilated_edges > 0).float()
...
class PointRend_try(nn.Module):
    ...
    def forward_train(self, image_path,image_embedding,  # [1,256,H_img,W_img]
                      mask_logits,  # [B,1,H_mask,W_mask]
                      scaled_h, scaled_w, pos_random):
        # 1.点采样
        sample_idx, sample_coords, boundry_idx, boundry_coords = sampling_points(image_path,mask_logits,
    ...
    def forward_eval(self, image_embedding,  # [1,256,H_img,W_img]
        ...
        sample_idx, sample_coords, boundry_idx, boundry_coords = sampling_points(mask_logits, max(mask_logits.shape[-1],
                                                                                 ...
                                                                                 False)  # list <=n（<=n,2）
        #end_time_temp = time.time()
        #print(f'record 11 :{end_time_temp-start_time_temp}')
        
        #start_time_temp = time.time()
        mask_binary = (mask_logits > 0.0).float()  # [B,1,H_mask,W_mask] 0-1
        b_mask, c_mask, h_mask, w_mask = mask_logits.shape
        #end_time_temp = time.time()
        #print(f'record 22 :{end_time_temp-start_time_temp}')
        
        #start_time_temp = time.time()
        up_features = self.upsample(image_embedding)  # [1, 32, 1024, 1024]
        up_features = up_features[..., : scaled_h, : scaled_w].contiguous()
        #end_time_temp = time.time()
        #print(f'record 33 :{end_time_temp-start_time_temp}')
        
        #start_time_temp = time.time()
        if up_features.shape[-2:] != mask_logits.shape[-2:]:
            up_features = F.interpolate(up_features, size=(h_mask, w_mask), mode='bilinear',align_corners=False)  # [1, 32, H_mask, W_mask]
        #end_time_temp = time.time()
        #print(f'record 44 :{end_time_temp-start_time_temp}')
