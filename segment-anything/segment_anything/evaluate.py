...
def calculate_f1(pred_mask, gt_mask):
    ...
    f1_scores = []
    for i in range(b):
        f1 = f1_metric(pred_flat[i], gt_flat[i])
        f1_scores.append(f1.item())

    # 计算整个批量的平均 F1 值
    average_f1 = sum(f1_scores) / b
    #print(f"平均 F1 值: {average_f1}")
    return average_f1
...
def init_model(image_encoder_pth=None):
    ...
    #checkpoint_path = os.path.join(current_dir, "checkpoint_bbbb","sam_epoch_5.pth")
...
def evaluate(dataset_folder_path,batch_size,log_Path):
    def recording_log(info):
        # 以写入模式打开文件，如果文件不存在则创建
        with open(log_Path, 'a') as file:
            # 遍历训练信息列表，逐行写入文件
            file.write(info + '\n')

    recording_log('model_type = vit_h')
    sam=init_model()
    transform = ResizeLongestSide(sam.image_encoder.img_size)  # 创建一个 ResizeLongestSide 实例，用于将图像的最长边调整为模型期望的尺寸
        ...
        for index, image_filename in enumerate(val_image_files, start=1):
            ...
            if os.path.exists(json_file_path):
                ...
                for batch_idx, (point_tensor, gt_mask_tensor) in enumerate(dataloader, start=1):
                    ...
                    f1=calculate_f1(masks.squeeze(1) > 0.0, gt_mask_tensor)
                    total_f1 += f1


                    print(f'evaluate ==> file:{index}/{num_val}  batch:{batch_idx}/{total_batch}, focal_loss: {focal}, dice_loss: {dice}, final_loss:{loss}, accuracy:{accuracy}, iou:{iou}, f1:{f1}')
                    recording_log(f'evaluate ==> file:{index}/{num_val}  batch:{batch_idx}/{total_batch}, focal_loss: {focal}, dice_loss: {dice}, final_loss:{loss}, accuracy:{accuracy}, iou:{iou}, f1:{f1}')

    avg_focal_loss = total_focal_loss / total_num
    avg_dice_loss = total_dice_loss / total_num
    ...
    print(f'Evaluate ==> Focal_loss: {avg_focal_loss}, Dice_loss: {avg_dice_loss}, Final_loss:{avg_val_loss},Accuracy: {avg_accuracy}, IoU: {avg_iou}, F1:{avg_f1}')
    recording_log(f'Evaluate ==> Focal_loss: {avg_focal_loss}, Dice_loss: {avg_dice_loss}, Final_loss:{avg_val_loss},Accuracy: {avg_accuracy}, IoU: {avg_iou}, F1:{avg_f1}')
...
log_path = os.path.join(current_dir, "evaluate_log_h.txt")
evaluate(datasetPath,16,log_path)
