...
from dataset_voc import PascalVOCImagePathDataset, PascalVOCInstanceSegDataset


def evaluate(batch_size):
    logging.basicConfig(filename='evaluate_voc_combine.log', level=logging.INFO,format='%(asctime)s - %(levelname)s - %(message)s')
    ...
    dataset_folder_path = r'/root/autodl-tmp/VOC2012'
    image_name_txt_path=os.path.join(dataset_folder_path,'ImageSets','Segmentation','val.txt')
        ...
        for image_index, (image_path, instance_seg_path, annotation_path) in tqdm(enumerate(path_dataloader, start=1), total=len(path_dataloader),desc=f'Evaluate ==> '):
            ...
            for batch_idx, (bbox_tensor, rbox_tensor, gt_mask_tensor) in enumerate(dataloader,start=1):  # point_tensor:b12    gt_mask_tensor:bhw
                if image_embeddings is None:
                    image_embeddings,input_size,original_size=process_single_image(sam, image_path[0])
                ...
                #masks = process_single_bbox(sam, image_path[0], bbox_tensor).cpu()  # bchw
                #masks = process_single(sam, image_path[0], bbox_tensor, rbox_tensor).cpu()  # bchw
                # 计算损失
                ...
                total_iou += iou
                print(f'Evaluate ==> file:{image_index}/{num_val}  batch:{batch_idx}/{len(dataloader)}, focal_loss: {focal}, dice_loss: {dice}, final_loss:{loss}, accuracy:{accuracy}, iou:{iou}')
                logging.info(f'Evaluate ==> file:{image_index}/{num_val}  batch:{batch_idx}/{len(dataloader)}, focal_loss: {focal}, dice_loss: {dice}, final_loss:{loss}, accuracy:{accuracy}, iou:{iou}')
                """
                # 可视化部分
