...
class ResizeLongestSide:
    ...
    def apply_image_return_target_size(self, image: np.ndarray) -> np.ndarray:
        ...
        """
        target_size = self.get_preprocess_shape(image.shape[0], image.shape[1], self.target_length)# 计算调整后的目标尺寸
        return np.array(resize(to_pil_image(image), target_size)),target_size # 将 NumPy 数组转换为 PIL 图像，调整大小，再转换回 NumPy 数组
    ...
    def apply_boxes_torch(
        ...
        """
        boxes = self.apply_coords_torch(boxes.reshape(-1, 2, 2), original_size)# 将框的张量重塑为 Bx2x2 的形状，调用 apply_coords_torch 方法调整坐标
