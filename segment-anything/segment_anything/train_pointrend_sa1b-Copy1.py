...
from pointrend_test import PointRend, PointHead, point_sample, StandardPointHead, PointHead_try, PointRend_try, \
    point_sample_by_idx
...
def train(epochs, batch_size, save_Path, unique_label,train_attempt):
    ...
    train_attempt_logger = init_logger(f"train_attempt_sa1b_pointrend-{unique_label}-6.log")

    train_attempt_logger.info(f"\n===== 开始第 {train_attempt} 次训练尝试 =====")
    prev_iou_history = []
    ...
    # 定义优化器，只优化可训练的参数
    optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, pointRend.parameters()), lr=1e-6, weight_decay=0.01)
    dice_loss = DiceLoss()
    ...
    train_dataset_folder_path = r'/root/autodl-tmp/SA-1B/train-1000'
    ...
    """
    if num_train > 0:
        save_backup_file("train_pointrend_sa1b.py",
                         f"lr4_origin_ConvTranspose2d_32_predicter_Linear124_2dim_norm_dropout2_1000_{train_attempt}-{unique_label}")
        save_backup_file("pointrend_test.py",
                         f"lr4_origin_ConvTranspose2d_32_predicter_Linear124_2dim_norm_dropout2_1000_{train_attempt}-{unique_label}")
    ...
    for epoch in range(epochs):
        ...
        loss = 0
        # 遍历所有图像文件
        for image_index, (image_filename, data) in tqdm(enumerate(zip(train_image_files, train_jsondata), start=1),
            ...
            for batch_idx, (bbox_tensor, rbox_tensor, gt_mask_tensor) in enumerate(dataloader,
                ...
                with torch.no_grad():
                    ...
                    if image_embeddings is None:
                        image_embeddings, input_size, original_size, target_size = process_single_image(sam,
                                                                                                        image_file_path)
                    low_res_masks = process_single_mask(sam, image_embeddings, input_size, original_size, bbox_tensor,
                                                        rbox_tensor)
                ...
                result_origin = pointRend.forward_train(image_embeddings, pred_masks_origin, input_size[0],
                ...
                print(
                    f'Epoch {epoch + 1}/{epochs}, file:{image_index}/{num_train}  batch:{batch_idx}/{len(dataloader)}, Gradient norm = {total_norm}')
                ...
                print(
                    f'TRAIN ==> Epoch {epoch + 1}/{epochs}, file:{image_index}/{num_train}  batch:{batch_idx}/{len(dataloader)}, loss_1024: {loss_1024}, loss_origin: {loss_origin},dice_1024: {dice_1024}, dice_origin: {dice_origin},focal_1024: {focal_1024}, focal_origin: {focal_origin}, final_loss:{loss}')
        ...
        train_avg_logger.info(
            f'TRAIN ==> Epoch {epoch + 1}/{epochs}, Train Loss_1024: {avg_train_loss_1024}, Train Loss_origin: {avg_train_loss_origin},Train avg_train_dice_1024: {avg_train_dice_1024},Train avg_train_dice_origin: {avg_train_dice_origin},Train avg_train_focal_1024: {avg_train_focal_1024},Train avg_train_focal_origin: {avg_train_focal_origin}, Train Loss: {avg_train_loss}')
        train_attempt_logger.info(f'当前总训练次数：{train_attempt}     TRAIN ==> Epoch {epoch + 1}/{epochs}, Train Loss_1024: {avg_train_loss_1024}, Train Loss_origin: {avg_train_loss_origin},Train avg_train_dice_1024: {avg_train_dice_1024},Train avg_train_dice_origin: {avg_train_dice_origin},Train avg_train_focal_1024: {avg_train_focal_1024},Train avg_train_focal_origin: {avg_train_focal_origin}, Train Loss: {avg_train_loss}')
        ...
        total_num = 0
        with torch.no_grad():
            for image_index, (image_filename, data) in tqdm(enumerate(zip(val_image_files, val_jsondata), start=1),
                ...
                for batch_idx, (bbox_tensor, rbox_tensor, gt_mask_tensor) in enumerate(dataloader,
                    ...
                    # 1024
                    pred_masks_1024 = F.interpolate(low_res_masks,
                                                    (sam.image_encoder.img_size, sam.image_encoder.img_size),
                                                    mode="bilinear", align_corners=False, )  # b 1 1024 1024
                    pred_masks_1024 = pred_masks_1024[..., : input_size[0], : input_size[1]].contiguous()
                    result_1024 = pointRend.forward_eval(image_embeddings, pred_masks_1024, input_size[0],
                                                         input_size[1], pos_random)

                    gt_mask_tensor_1024 = (F.interpolate(gt_mask_tensor.unsqueeze(1), size=pred_masks_1024.shape[-2:],
                                                         mode='bilinear') > 0.0).float()  # b1hw
                    ...
                    # origin
                    pred_masks_origin = F.interpolate(pred_masks_1024, original_size, mode="bilinear",
                                                      align_corners=False)
                    ...
                    print(
                        f'VAL ==> Epoch {epoch + 1}/{epochs}, file:{image_index}/{num_val}  batch:{batch_idx}/{len(dataloader)}, loss_1024: {loss_1024}, loss_origin: {loss_origin},dice_1024: {dice_1024}, dice_origin: {dice_origin},focal_1024: {focal_1024}, focal_origin: {focal_origin},  final_loss:{loss}, iou:{iou}')
                    training_logger.info(
                        f'VAL ==> Epoch {epoch + 1}/{epochs}, file:{image_index}/{num_val}  batch:{batch_idx}/{len(dataloader)}, loss_1024: {loss_1024}, loss_origin: {loss_origin},dice_1024: {dice_1024}, dice_origin: {dice_origin},focal_1024: {focal_1024}, focal_origin: {focal_origin},  final_loss:{loss}, iou:{iou}')
        ...
        print(
            f'VAL ==> Epoch {epoch + 1}/{epochs}, val Loss_1024: {avg_val_loss_1024}, val Loss_origin: {avg_val_loss_origin},Train avg_val_dice_1024: {avg_val_dice_1024},Train avg_val_dice_origin: {avg_val_dice_origin},Train avg_val_focal_1024: {avg_val_focal_1024},Train avg_val_focal_origin: {avg_val_focal_origin}, val Loss: {avg_val_loss} IoU: {avg_iou}')
        val_avg_logger.info(
            f'VAL ==> Epoch {epoch + 1}/{epochs}, val Loss_1024: {avg_val_loss_1024}, val Loss_origin: {avg_val_loss_origin},Train avg_val_dice_1024: {avg_val_dice_1024},Train avg_val_dice_origin: {avg_val_dice_origin},Train avg_val_focal_1024: {avg_val_focal_1024},Train avg_val_focal_origin: {avg_val_focal_origin}, val Loss: {avg_val_loss} IoU: {avg_iou}')
        train_attempt_logger.info(f'当前总训练次数：{train_attempt}     VAL ==> Epoch {epoch + 1}/{epochs}, val Loss_1024: {avg_val_loss_1024}, val Loss_origin: {avg_val_loss_origin},Train avg_val_dice_1024: {avg_val_dice_1024},Train avg_val_dice_origin: {avg_val_dice_origin},Train avg_val_focal_1024: {avg_val_focal_1024},Train avg_val_focal_origin: {avg_val_focal_origin}, val Loss: {avg_val_loss} IoU: {avg_iou}')
        ...
        if num_train > 0:
            ...
            checkpoint_path = os.path.join(save_dir,
                                           f'pointRend_sam_epoch_{epoch + 1}_lr4_origin_ConvTranspose2d_32_predicter_Linear124_2dim_norm_dropout2_1000_{train_attempt}-{unique_label}.pth')
            ...
            if avg_iou > best_iou:
                ...
                # 保存图像编码器的参数
                checkpoint_path = os.path.join(save_dir,
                                               f'pointRend_sam_best_epoch_lr4_origin_ConvTranspose2d_32_predicter_Linear124_2dim_norm_dropout2_1000_{train_attempt}-{unique_label}.pth')
                torch.save(pointRend.state_dict(), checkpoint_path)
                print(f'Saved pointRend parameters to {checkpoint_path}')
                training_logger.info(f'Saved pointRend parameters to {checkpoint_path}')

        prev_iou_history.append(avg_iou)
        if len(prev_iou_history) > 3:
            prev_iou_history.pop(0)  # 保持最近两次记录
        # 检查连续两次下降
        if len(prev_iou_history) == 3 and prev_iou_history[-1] < prev_iou_history[-2] and prev_iou_history[-2] < prev_iou_history[-3]:
            train_attempt_logger.info(f"警告：验证IoU连续两次下降，准备重启训练")
            train_attempt_logger.info(f"第 {train_attempt} 次训练提前中断，准备新的训练尝试，本次最佳IoU={best_iou:.4f} 于第 {best_epoch}  epoch")
            break

    print(
        f'训练已完成 Epoch：{epochs}, Batch: {batch_size}, num_train: {num_train}, num_val: {num_val}, best_epoch:{best_epoch}, best_loss:{best_loss}, best_iou:{best_iou} ')
    training_logger.info(
        f'训练已完成 Epoch：{epochs}, Batch: {batch_size}, num_train: {num_train}, num_val: {num_val}, best_epoch:{best_epoch}, best_loss:{best_loss}, best_iou:{best_iou} ')
...
train_attempt_max = 20
train_attempt = 0

while train_attempt < train_attempt_max:
    train_attempt += 1
    train(20, 16, save_Path, unique_label,train_attempt)
