...
def train(dataset_folder_path,epochs,batch_size,save_Path,log_Path):
    ...
    for epoch in range(epochs):
        ...
        for index, image_filename in enumerate(train_image_files, start=1):
            ...
            if os.path.exists(json_file_path):
                ...
                for batch_idx, (point_tensor, gt_mask_tensor) in enumerate(dataloader, start=1):#point_tensor:b12    gt_mask_tensor:bhw
                    ...
                    recording_log(f'TRAIN ==> Epoch {epoch + 1}/{epochs}, file:{index}/{num_train}  batch:{batch_idx}/{total_batch}, focal_loss: {focal}, dice_loss: {dice}, final_loss:{loss}, accuracy:{accuracy}, iou:{iou}')
            ...
            for index, image_filename in enumerate(val_image_files, start=1):
                ...
                if os.path.exists(json_file_path):
                    ...
                    for batch_idx, (point_tensor, gt_mask_tensor) in enumerate(dataloader, start=1):
                        ...
                        recording_log(f'VAL ==> Epoch {epoch + 1}/{epochs}, file:{index}/{num_val}  batch:{batch_idx}/{total_batch}, focal_loss: {focal}, dice_loss: {dice}, final_loss:{loss}, accuracy:{accuracy}, iou:{iou}')
        ...
        recording_log(f'VAL ==> Epoch {epoch + 1}/{epochs}, Val Loss: {avg_val_loss}, Accuracy: {avg_accuracy}, IoU: {avg_iou}')

        # 保存图像编码器的参数
        #checkpoint_path = os.path.join(save_dir, f'image_encoder_epoch_{epoch + 1}.pth')
        checkpoint_path = os.path.join(save_dir, f'sam_epoch_{epoch + 1}.pth')
        ...
        recording_log(f'Saved sam parameters to {checkpoint_path}')


        if avg_val_loss<best_loss and avg_iou>best_iou:
            best_epoch=epoch + 1
            ...
            checkpoint_path = os.path.join(save_dir, f'sam_best.pth')
            #torch.save(sam.image_encoder.state_dict(), checkpoint_path)
            torch.save(sam.state_dict(), checkpoint_path)
    ...
    recording_log(f'训练已完成 Epoch：{epochs}, Batch: {batch_size}, num_train: {num_train}, num_val: {num_val}, best_epoch:{best_epoch}, best_loss:{best_loss}, best_accuracy:{best_accuracy}, best_iou:{best_iou} ')
...
save_Path= os.path.join(current_dir, "checkpoint_bbbb")
log_path = os.path.join(current_dir, "training_log_bbbb.txt")
train(datasetPath,5,16,save_Path,log_path)
