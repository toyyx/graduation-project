...
class StandardPointHead(nn.Module):
    ...
    def __init__(self):
        ...
        self.boundry_embeddings = nn.Embedding(1, input_channels)  # 创建点嵌入层列表，每个点嵌入层是一个Embedding层，嵌入维度为embed_dim
        self.point_embeddings = nn.Embedding(1, input_channels)  # 创建点嵌入层列表，每个点嵌入层是一个Embedding层，嵌入维度为embed_dim


        self.self_attn = NormalAttention(input_channels, input_channels,input_channels, num_heads)
        ...
        self.mlp1 = nn.Sequential(
            nn.Linear(input_channels, 8*input_channels),
            nn.ReLU(),
            nn.Linear(8*input_channels, input_channels),
        )
        ...
        """
        self.transformer=TransformerBoundary(input_channels,num_heads)
    ...
    def forward(self, img_samples, img_samples_boundrys, pos_samples, pos_samples_boundrys, mask_samples):
        ...
        if isinstance(img_samples, torch.Tensor) and isinstance(pos_samples,
            ...
            # 处理 fine_grained_features 和 coarse_features 为列表的情况
            output_list = []
            for img_sample, img_samples_boundry, pos_sample, pos_samples_boundry, mask_sample in zip(img_samples,
                ...
                pos_sample = pos_sample.unsqueeze(0)+self.point_embeddings.weight
                pos_samples_boundry = pos_samples_boundry.unsqueeze(0)+self.boundry_embeddings.weight
                mask_sample = mask_sample.unsqueeze(0)

                query = img_sample
                key = img_samples_boundry
                value = mask_sample  # bn1
                ...
                q = query + pos_sample
                k = key + pos_samples_boundry
                attn_out= self.cro_attn2(query=k, key=q, value=query)
                key=key+attn_out
                key=self.norm4(key)
...
class PointRend_try(nn.Module):
    ...
    def forward_train(self, image_embedding,  # [1,256,H_img,W_img]
        ...
        mask_samples = point_sample_by_idx(mask_logits, boundry_idx)  # [B,N,1]

        # main_expanded = main_feature.unsqueeze(1).expand(-1, N, -1)# [B,N,32]

        rend = self.head(img_samples, img_samples_boundry, pos_samples, pos_samples_boundry, mask_samples)  # bn1
    ...
    def forward_eval(self, image_embedding,  # [1,256,H_img,W_img]
        ...
        mask_samples = point_sample_by_idx(mask_logits, boundry_idx)  # list <=N,h_dim
        # main_expanded = [main_feature[i].unsqueeze(0).expand(sample_coords[i].shape[0], -1) for i in range(len(sample_coords))]  #list <=N,32

        rend = self.head(img_samples, img_samples_boundry, pos_samples, pos_samples_boundry, mask_samples)  # list <=n,1
...
class LogitAttention(nn.Module):
    def __init__(self, query_dim, key_dim, num_heads):
        ...
        self.q_proj = nn.Linear(query_dim, query_dim)
        self.k_proj = nn.Linear(key_dim, query_dim)
        self.out_proj = nn.Linear(num_heads, 1)  # 定义输出的投影层，将内部维度的向量投影回输入嵌入向量的维度
    ...
    def forward(self, query, key, value):
        ...
        q = self._separate_heads(q, self.num_heads)  # B x N_heads x N_tokens x C_per_head
        k = self._separate_heads(k, self.num_heads)  # B x N_heads x N_tokens x C_per_head
        v = value.unsqueeze(2).expand(-1, -1, self.num_heads, -1).transpose(1, 2)  # b N_heads N_tokens 1
