...
def train(epochs, batch_size, save_Path):
    ...
    if num_train>0:
        save_backup_file("train_pointrend_sa1b.py","lr4_origin_ConvTranspose2d_32_predicter_Linear124_2dim_norm_dropout2_val")
        save_backup_file("pointrend_test.py","lr4_origin_ConvTranspose2d_32_predicter_Linear124_2dim_norm_dropout2_val")
    ...
    for epoch in range(epochs):
        ...
        if num_train>0:
            # 保存图像编码器的参数
            checkpoint_path = os.path.join(save_dir, f'pointRend_sam_epoch_{epoch + 1}_lr4_origin_ConvTranspose2d_32_predicter_Linear124_2dim_norm_dropout2_val.pth')
            ...
            if avg_iou > best_iou:
                ...
                best_iou = avg_iou
                # 保存图像编码器的参数
                checkpoint_path = os.path.join(save_dir, f'pointRend_sam_best_epoch_lr4_origin_ConvTranspose2d_32_predicter_Linear124_2dim_norm_dropout2_val.pth')
                torch.save(pointRend.state_dict(), checkpoint_path)
                print(f'Saved pointRend parameters to {checkpoint_path}')
                training_logger.info(f'Saved pointRend parameters to {checkpoint_path}')
