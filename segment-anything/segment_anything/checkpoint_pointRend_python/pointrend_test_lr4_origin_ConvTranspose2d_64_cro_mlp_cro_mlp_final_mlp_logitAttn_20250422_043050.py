...
class StandardPointHead(nn.Module):
    ...
    def __init__(self):
        ...
        self.cro_attn1 = NormalAttention(input_channels, input_channels,input_channels, num_heads)
        self.norm1 = nn.LayerNorm(input_channels)  # 创建一个层归一化层，用于对自注意力层的输出进行归一化
        ...
        self.norm2 = nn.LayerNorm(input_channels)  # 创建一个层归一化层，用于对该交叉注意力层的输出进行归一化


        self.cro_attn2 = NormalAttention(input_channels, input_channels, input_channels, num_heads)
        self.norm3 = nn.LayerNorm(input_channels)  # 创建一个层归一化层，用于对该交叉注意力层的输出进行归一化
        ...
        self.norm4 = nn.LayerNorm(input_channels)  # 创建一个层归一化层，用于对该交叉注意力层的输出进行归一化

        self.final_attn = LogitAttention(input_channels,input_channels,num_heads)
        self.norm5 = nn.LayerNorm(input_channels)  # 创建一个层归一化层，用于对该交叉注意力层的输出进行归一化
        ...
        self.norm6 = nn.LayerNorm(input_channels)  # 创建一个层归一化层，用于对该交叉注意力层的输出进行归一化

        self.logit_attn=FinalAttention(input_channels,input_channels,output_channels)

        """
        self.predicter = nn.Sequential(
            nn.Linear(2*input_channels+1+1, 2*input_channels+1+1),
            nn.ReLU(),
            nn.Linear(2*input_channels+1+1, 2*input_channels+1+1),
            nn.ReLU(),
            nn.Linear(2*input_channels+1+1, 1),
        )
        """
        

    def forward(self, img_samples, img_samples_boundrys, pos_samples, pos_samples_boundrys, mask_samples,mask_samples_boundrys):
        ...
        if isinstance(img_samples, torch.Tensor) and isinstance(pos_samples,
            ...
            # 处理 fine_grained_features 和 coarse_features 为列表的情况
            output_list = []
            for img_sample, img_samples_boundry, pos_sample, pos_samples_boundry, mask_sample,mask_samples_boundry in zip(img_samples,
                ...
                query = img_sample.clone()
                key = img_samples_boundry
                value = mask_samples_boundry  # bn1


                q = query + pos_sample
                k = key + pos_samples_boundry
                attn_out = self.cro_attn1(query=q, key=k, value=key)
                query = query+attn_out
                query = self.norm1(query)

                mlp_out = self.mlp1(query)
                query = query+mlp_out
                query = self.norm2(query)

                q = query + pos_sample
                k = key + pos_samples_boundry
                attn_out= self.cro_attn2(query=q, key=k, value=key)
                query=query+attn_out
                query=self.norm3(query)

                mlp_out = self.mlp2(query)
                query = query + mlp_out
                query = self.norm4(query)

                q = query + pos_sample
                k = key + pos_samples_boundry
                attn_out=self.final_attn(query=q, key=k, value=value)
                query = query + attn_out
                query = self.norm5(query)

                mlp_out = self.mlp3(query)
                query=query+mlp_out
                query=self.norm6(query)

                q = query + pos_sample
                k = key + pos_samples_boundry
                attn_out = self.logit_attn(query=q, key=k, value=value)#bn1

                """
                input=torch.cat([img_sample, query, mask_sample,attn_out], dim=-1)#b n 64+64+1+1
                output=self.predicter(input)
                """
                

                output_list.append(attn_out.squeeze(0))



                #mlp_out = self.cro_attn(query, key, value)

                """
                attn_out=self.transformer(query.permute(1,0,2),key.permute(1,0,2),value.permute(1,0,2))#nbc
                query=attn_out.permute(1,0,2)#query=attn_out
                query=self.norm1(query)

                mlp_out=self.mlp(query)#bn1
                """
            ...
            """
            query=img_samples
            key=img_samples_boundrys
            value=mask_samples#bn1

            query=query+pos_samples#bnc
            key=key+pos_samples_boundrys#bNc

            attn_out=self.transformer(query.permute(1,0,2),key.permute(1,0,2),value.permute(1,0,2))#nbc
            query=query+attn_out.permute(1,0,2)#query=attn_out
            query=self.norm1(query)
            ...
            for img_sample, img_samples_boundry, pos_sample, pos_samples_boundry, mask_sample,mask_samples_boundry in zip(img_samples,
                ...
                #output_list.append(mlp_out.squeeze(0))
            return output_list  # list <=n，1
        else:
            raise ValueError("fine_grained_features 和 coarse_features 必须同时为张量或列表")
...
class PointRend_try(nn.Module):
    ...
    def forward_train(self, image_embedding,  # [1,256,H_img,W_img]
        ...
        rend = self.head(img_samples, img_samples_boundry, pos_samples, pos_samples_boundry, mask_samples,mask_samples_boundry)  # bn1
    ...
    def forward_eval(self, image_embedding,  # [1,256,H_img,W_img]
        ...
        rend = self.head(img_samples, img_samples_boundry, pos_samples, pos_samples_boundry, mask_samples,mask_samples_boundry)  # list <=n,1
...
class LogitAttention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        _, _, _, c_per_head = q.shape  # 获取每个头的通道数
        attn = q @ k.permute(0, 1, 3, 2)  # B x N_heads x N_tokens x N_tokens  # 计算查询和键的点积，得到注意力分数矩阵 bnc * bcN=bnN *  bN1=bn1
        attn = attn / math.sqrt(c_per_head)  # 对注意力分数矩阵进行缩放，除以每个头通道数的平方根，防止点积结果过大
        attn = torch.softmax(attn, dim=-1)  # 对注意力分数矩阵在最后一个维度上进行 softmax 操作，得到注意力权重
