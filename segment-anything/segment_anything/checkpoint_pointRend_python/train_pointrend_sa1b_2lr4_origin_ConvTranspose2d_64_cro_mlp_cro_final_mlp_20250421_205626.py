...
def train(epochs, batch_size, save_Path):
    ...
    sam.eval()
    pointRend_checkpoint_path='./checkpoint_pointRend/pointRend_sam_epoch_10_lr4_origin_ConvTranspose2d_64_cro_mlp_cro_final_mlp.pth'
    #pointRend_checkpoint_path = None
    pointRend = init_pointrend(pointRend_checkpoint_path)
    pos_random=PositionEmbeddingRandom(64)

    # 定义优化器，只优化可训练的参数
    optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, pointRend.parameters()), lr=2e-4, weight_decay=0.01)
    dice_loss = DiceLoss()
    ...
    if num_train>0:
        save_backup_file("train_pointrend_sa1b.py","2lr4_origin_ConvTranspose2d_64_cro_mlp_cro_final_mlp")
        save_backup_file("pointrend_test.py","2lr4_origin_ConvTranspose2d_64_cro_mlp_cro_final_mlp")
    ...
    for epoch in range(epochs):
        ...
        if num_train>0:
            # 保存图像编码器的参数
            checkpoint_path = os.path.join(save_dir, f'pointRend_sam_epoch_{epoch + 1+10}_2lr4_origin_ConvTranspose2d_64_cro_mlp_cro_final_mlp.pth')
            ...
            if avg_iou > best_iou:
                ...
                best_iou = avg_iou
                # 保存图像编码器的参数
                checkpoint_path = os.path.join(save_dir, f'pointRend_sam_best_epoch_2lr4_origin_ConvTranspose2d_64_cro_mlp_cro_final_mlp.pth')
                torch.save(pointRend.state_dict(), checkpoint_path)
                print(f'Saved pointRend parameters to {checkpoint_path}')
                training_logger.info(f'Saved pointRend parameters to {checkpoint_path}')
