...
def process_single_mask(sam, image_embeddings, input_size, original_size, bbox_tensor, rbox_tensor):
    ...
    low_res_masks, iou_predictions, src = sam.mask_decoder(
        image_embeddings=image_embeddings,
        image_pe=sam.prompt_encoder.get_dense_pe(),
        sparse_prompt_embeddings=sparse_embeddings,
        dense_prompt_embeddings=dense_embeddings,
...
def train(epochs, batch_size, save_Path):
    ...
    sam.eval()
    #pointRend_checkpoint_path='./checkpoint_pointRend/pointRend_sam_epoch_7_lr4_origin_ConvTranspose2d_32_transformer.pth'
    ...
    if num_train>0:
        save_backup_file("train_pointrend_sa1b.py","lr4_origin_ConvTranspose2d_64_cro_mlp_cro_mlp_final_mlp_logitAttn_predicter_nolinear")
        save_backup_file("pointrend_test.py","lr4_origin_ConvTranspose2d_64_cro_mlp_cro_mlp_final_mlp_logitAttn_predicter_nolinear")
    ...
    for epoch in range(epochs):
        ...
        for image_index, (image_filename, data) in tqdm(enumerate(zip(train_image_files, train_jsondata), start=1),total=len(train_image_files),desc=f'Training ==> Epoch:{epoch + 1}/{epochs}'):
            ...
            for batch_idx, (bbox_tensor, rbox_tensor, gt_mask_tensor) in enumerate(dataloader,start=1):  # point_tensor:b12    gt_mask_tensor:bhw
                with torch.no_grad():
                    ...
                    low_res_masks,src = process_single_mask(sam, image_embeddings, input_size, original_size, bbox_tensor,rbox_tensor)
        ...
        if num_train>0:
            # 保存图像编码器的参数
            checkpoint_path = os.path.join(save_dir, f'pointRend_sam_epoch_{epoch + 1}_lr4_origin_ConvTranspose2d_64_cro_mlp_cro_mlp_final_mlp_logitAttn_predicter_nolinear.pth')
            ...
            if avg_iou > best_iou:
                ...
                best_iou = avg_iou
                # 保存图像编码器的参数
                checkpoint_path = os.path.join(save_dir, f'pointRend_sam_best_epoch_lr4_origin_ConvTranspose2d_64_cro_mlp_cro_mlp_final_mlp_logitAttn_predicter_nolinear.pth')
                torch.save(pointRend.state_dict(), checkpoint_path)
                print(f'Saved pointRend parameters to {checkpoint_path}')
                training_logger.info(f'Saved pointRend parameters to {checkpoint_path}')
