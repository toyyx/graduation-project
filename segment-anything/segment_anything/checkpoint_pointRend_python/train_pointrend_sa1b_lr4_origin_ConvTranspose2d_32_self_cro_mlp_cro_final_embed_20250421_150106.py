...
def train(epochs, batch_size, save_Path):
    ...
    pos_random=PositionEmbeddingRandom(32)
    ...
    if num_train>0:
        save_backup_file("train_pointrend_sa1b.py","lr4_origin_ConvTranspose2d_32_self_cro_mlp_cro_final_embed")
        save_backup_file("pointrend_test.py","lr4_origin_ConvTranspose2d_32_self_cro_mlp_cro_final_embed")
    ...
    for epoch in range(epochs):
        ...
        if num_train>0:
            # 保存图像编码器的参数
            checkpoint_path = os.path.join(save_dir, f'pointRend_sam_epoch_{epoch + 1}_lr4_origin_ConvTranspose2d_32_self_cro_mlp_cro_final_embed.pth')
            ...
            if avg_iou > best_iou:
                ...
                best_iou = avg_iou
                # 保存图像编码器的参数
                checkpoint_path = os.path.join(save_dir, f'pointRend_sam_best_epoch_lr4_origin_ConvTranspose2d_32_self_cro_mlp_cro_final_embed.pth')
                torch.save(pointRend.state_dict(), checkpoint_path)
                print(f'Saved pointRend parameters to {checkpoint_path}')
                training_logger.info(f'Saved pointRend parameters to {checkpoint_path}')
