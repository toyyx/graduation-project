...
def sampling_points(pred_mask, N, k=3, beta=0.75, training=True):
    ...
    if not training:
        ...
        important_indices, important_coords, boundry_indices, boundry_coords = edge_topk_sampling_with_boundry(edge_masks, uncertainty_map, num_important,num_important)  # list （<=topk，2）
    ...
    important_indices, important_coords, boundry_indices, boundry_coords = edge_topk_sampling_with_boundry(edge_masks, uncertainty_map, num_important,num_important)  # list <=topk （<=topk，2）
...
class StandardPointHead(nn.Module):
    ...
    def __init__(self):
        ...
        self.cro_attn=CrossAttention(input_channels,input_channels,num_heads)
    ...
    def forward(self, img_samples, img_samples_boundrys,pos_samples,pos_samples_boundrys,mask_samples):
        ...
        if isinstance(img_samples, torch.Tensor) and isinstance(pos_samples,torch.Tensor):  # 处理 fine_grained_features 和 coarse_features 为张量的情况
            # 处理 fine_grained_features 和 coarse_features 为列表的情况
            output_list = []
            for img_sample, img_samples_boundry,pos_sample,pos_samples_boundry,mask_sample in zip(img_samples, img_samples_boundrys,pos_samples,pos_samples_boundrys,mask_samples):
                img_sample = img_sample.unsqueeze(0)
                ...
                query=query+pos_sample#bnc
                key=key+pos_samples_boundry#bNc
...
def edge_topk_sampling_with_boundry(edge_masks, uncertainty_map, topk,untopk):
    ...
    for i in range(b):
        ...
        effective_untopk = num_edge_points
...
class PointRend_try(nn.Module):
    def __init__(self, head, k=3, beta=0.75, training=True, feature_dim=256, hidden_dim=128,
        ...
        self.upsample = nn.Sequential(
            ...
            nn.GELU(), # 再次应用指定的激活函数
            nn.ConvTranspose2d(64, 32, kernel_size=2, stride=2), # 第二个反卷积层，将通道数从 transformer_dim // 4 减少到 transformer_dim // 8
            #LayerNorm2d(transformer_dim // 4), # 二维层归一化层，对通道维度进行归一化
            nn.GELU(), # 再次应用指定的激活函数
        )
    ...
    def forward_train(self, image_embedding,  # [1,256,H_img,W_img]
        ...
        #1.点采样
        sample_idx,sample_coords,boundry_idx,boundry_coords = sampling_points(mask_logits,(mask_logits.shape[-1] // 64) * (mask_logits.shape[-2] // 64), self.k, self.beta, True)  #bn bn2 0-1
    ...
    def forward_eval(self, image_embedding,  # [1,256,H_img,W_img]
                     mask_logits,  # [B,1,H_mask,W_mask]
                     scaled_h, scaled_w,pos_random):
        sample_idx, sample_coords,boundry_idx,boundry_coords = sampling_points(mask_logits, max(mask_logits.shape[-1],mask_logits.shape[-2])//32,self.k, self.beta, False)  # list <=n（<=n,2）
