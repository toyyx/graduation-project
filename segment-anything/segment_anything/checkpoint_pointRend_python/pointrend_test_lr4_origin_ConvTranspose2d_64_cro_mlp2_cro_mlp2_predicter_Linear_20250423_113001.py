...
class StandardPointHead(nn.Module):
    ...
    def __init__(self):
        ...
        self.cro_attn1 = NormalAttention(query_dim=input_channels, key_dim=input_channels, value_dim=input_channels+1,hidden_dim=input_channels, num_heads=num_heads)
        ...
        self.cro_attn2 = NormalAttention(query_dim=input_channels, key_dim=input_channels, value_dim=input_channels+1,hidden_dim=input_channels, num_heads=num_heads)
        ...
        self.predicter = nn.Sequential(
            nn.Linear(2 * input_channels + 1 , 256 ),
            nn.GELU(),
            nn.Linear(256 , 128 ),
            nn.GELU(),
            nn.Linear(128 , 1),
    ...
    def forward(self, img_samples, img_samples_boundrys, pos_samples, pos_samples_boundrys, mask_samples,
        ...
        if isinstance(img_samples, torch.Tensor) and isinstance(pos_samples,
            ...
            for img_sample, img_samples_boundry, pos_sample, pos_samples_boundry, mask_sample, mask_samples_boundry in zip(
                ...
                key = img_samples_boundry.clone()
                value = mask_samples_boundry.clone()  # bn1

                q = query
                k = key
                attn_out = self.cro_attn1(query=q, key=k, value=torch.cat([key, value], dim=-1))
                ...
                q = query
                k = key
                attn_out = self.cro_attn2(query=q, key=k, value=torch.cat([key, value], dim=-1))
                query = query + attn_out
                ...
                input = torch.cat([img_sample, query, mask_sample], dim=-1)  # b n 64+64+1
...
class NormalAttention(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim,hidden_dim, num_heads):
        ...
        self.out_proj = nn.Linear(hidden_dim, query_dim)  # 定义输出的投影层，将内部维度的向量投影回输入嵌入向量的维度
