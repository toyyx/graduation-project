...
def evaluate(epochs, batch_size, save_Path):
    logging.basicConfig(filename='evaluate_coco_sam.log', level=logging.INFO,
    ...
    total_num = 0
    with torch.no_grad():
        for image_index, (img_path, annotations, w, h) in tqdm(enumerate(val_image_dataloader, start=1),
                                                               total=len(val_image_dataloader), desc=f'Evaluate ==> '):
            ...
            for batch_idx, (bbox_tensor, rbox_tensor, gt_mask_tensor) in enumerate(oneImage_dataloader,
                ...
                print(
                    f'Evaluate ==> file:{image_index}/{num_val}  batch:{batch_idx}/{len(oneImage_dataloader)}, iou:{iou}')
                logging.info(
                    f'Evaluate ==> file:{image_index}/{num_val}  batch:{batch_idx}/{len(oneImage_dataloader)}, iou:{iou}')
