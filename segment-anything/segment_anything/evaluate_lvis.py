    ...
    with torch.no_grad():
        ...
        for image_index, (image_path, img_info, anns) in tqdm(enumerate(image_dataloader, start=1),total=len(image_dataloader),desc=f'Evaluate ==> '):
            ...
            for batch_idx, (bbox_tensor, rbox_tensor, gt_mask_tensor) in enumerate(dataloader, start=1):  # point_tensor:b12    gt_mask_tensor:bhw
                ...
                #masks_new=keep_largest_connected_region((masks.squeeze(1) > 0.0).to(torch.uint8))
                
                #masks = process_single_bbox(sam, image_path, bbox_tensor).cpu()  # bchw
                #masks = process_single(sam, image_path, bbox_tensor, rbox_tensor).cpu()  # bchw
                # 计算损失
                ...
                #iou = calculate_iou(masks_new.squeeze(1) > 0.0, gt_mask_tensor)
