...
class PromptEncoder(nn.Module):# 定义提示编码器类，用于对输入到SAM的掩码解码器的提示进行编码
    def __init__(
        ...
        point_embeddings = [nn.Embedding(1, embed_dim) for i in range(self.num_point_embeddings)]# 创建点嵌入层列表，每个点嵌入层是一个Embedding层，嵌入维度为embed_dim
        ...
        rbox_point_embeddings = [nn.Embedding(1, embed_dim) for i in range(self.num_rbox_point_embeddings)]  # 创建点嵌入层列表，每个点嵌入层是一个Embedding层，嵌入维度为embed_dim
        ...
        bbox_point_embeddings_origin = [nn.Embedding(1, embed_dim) for i in range(self.num_bbox_point_embeddings_origin)]  # 创建点嵌入层列表，每个点嵌入层是一个Embedding层，嵌入维度为embed_dim
        ...
        rbox_point_embeddings_origin = [nn.Embedding(1, embed_dim) for i in range(self.num_rbox_point_embeddings_origin)]  # 创建点嵌入层列表，每个点嵌入层是一个Embedding层，嵌入维度为embed_dim
        ...
        self.mask_downscaling = nn.Sequential( # 定义掩码下采样模块，将输入掩码进行下采样并转换为嵌入维度
            nn.Conv2d(1, mask_in_chans // 4, kernel_size=2, stride=2), # 第一个卷积层，输入通道为1，输出通道为mask_in_chans的四分之一，卷积核大小为2，步长为2
            LayerNorm2d(mask_in_chans // 4), # 二维层归一化层，对卷积层的输出进行归一化
            activation(),# 激活函数层
            nn.Conv2d(mask_in_chans // 4, mask_in_chans, kernel_size=2, stride=2), # 第二个卷积层，输入通道为mask_in_chans的四分之一，输出通道为mask_in_chans，卷积核大小为2，步长为2
    ...
    def _embed_points(
        ...
        if pad:
            padding_point = torch.zeros((points.shape[0], 1, 2), device=points.device)# 创建一个全零的填充点张量，形状为 (batch_size, 1, 2)，表示在每个样本中添加一个填充点
            padding_label = -torch.ones((labels.shape[0], 1), device=labels.device) # 创建一个全 -1 的填充标签张量，形状为 (batch_size, 1)，表示填充点的标签
        ...
        point_embedding = self.pe_layer.forward_with_coords(points, self.input_image_size) # 调用位置编码层的 forward_with_coords 方法，根据点坐标和输入图像大小生成点的位置编码
        point_embedding[labels == -1] = 0.0 # 将标签为 -1 的点的嵌入设置为全零
        point_embedding[labels == -1] += self.not_a_point_embed.weight   # 为标签为 -1 的点的嵌入添加 not_a_point_embed 层的权重，以表示这不是一个有效的点
    ...
    def _embed_boxes(self, boxes: torch.Tensor) -> torch.Tensor: #输入的边界框张量，形状通常为 (batch_size, 4)，其中 4 表示边界框的 (x1, y1, x2, y2) 坐标
        ...
        corner_embedding = self.pe_layer.forward_with_coords(coords, self.input_image_size) # 调用位置编码层的 forward_with_coords 方法，根据边界框角点坐标和输入图像大小生成角点的位置编码
    ...
    def _embed_rboxes(self, rboxes: torch.Tensor) -> torch.Tensor: #输入的边界框张量，形状通常为 (batch_size, 4, 2)，其中 4 表示边界框的 (x1, y1, x2, y2) 坐标
        ...
        rbox_embedding = self.pe_layer.forward_with_coords(coords, self.input_image_size) # 调用位置编码层的 forward_with_coords 方法，根据边界框角点坐标和输入图像大小生成角点的位置编码  b,4,dim
    ...
    def _embed_bboxes_origin(self, bboxes: torch.Tensor) -> torch.Tensor: #输入的边界框张量，形状通常为 (batch_size, 4, 2)，其中 4 表示边界框的 (x1, y1, x2, y2) 坐标
        """Embeds rbox prompts.""" #对输入的边界框提示进行嵌入编码
        ...
        bbox_embedding_origin = self.pe_layer.forward_with_coords(coords, self.input_image_size) # 调用位置编码层的 forward_with_coords 方法，根据边界框角点坐标和输入图像大小生成角点的位置编码  b,2,dim
    ...
    def _embed_rboxes_origin(self, rboxes: torch.Tensor) -> torch.Tensor: #输入的边界框张量，形状通常为 (batch_size, 4, 2)，其中 4 表示边界框的 (x1, y1, x2, y2) 坐标
        """Embeds rbox prompts.""" #对输入的边界框提示进行嵌入编码
        boxes = rboxes + 0.5  # Shift to center of pixel # 将边界框坐标偏移 0.5，使其移动到像素的中心位置
        coords = boxes.reshape(-1, 4, 2) # 将边界框张量重塑为 (batch_size, 4, 2) 的形状，其中第二个维度的 4 表示边界框的4个角点
        rbox_embedding_origin = self.pe_layer.forward_with_coords(coords, self.input_image_size) # 调用位置编码层的 forward_with_coords 方法，根据边界框角点坐标和输入图像大小生成角点的位置编码  b,4,dim
    ...
    def _embed_masks(self, masks: torch.Tensor) -> torch.Tensor:#输入的掩码张量，形状通常为 (batch_size, 1, H, W)，其中 H 和 W 是掩码的高度和宽度
        """Embeds mask inputs."""#对输入的掩码进行嵌入编码
        mask_embedding = self.mask_downscaling(masks)# 将掩码张量输入到掩码下采样模块中进行下采样和特征转换
        return mask_embedding
    
    def _embed_masks_rbox_origin(self, rboxes: torch.Tensor) -> torch.Tensor:#输入的掩码张量，形状通常为 (batch_size, 1, H, W)，其中 H 和 W 是掩码的高度和宽度
        transform = ResizeLongestSide(self.image_embedding_size[0])
        rboxes = torch.tensor(transform.apply_rboxes(rboxes.cpu().numpy(), self.input_image_size),device=self._get_device())  # 对输入的点坐标进行变换，使其与模型输入的图像尺寸匹配
        """Embeds mask inputs."""#对输入的掩码进行嵌入编码
    ...
    def forward(
        ...
        """
        bs = self._get_batch_size(points, boxes,boxes_origin, rboxes,rboxes_origin, masks)  # 调用 _get_batch_size 方法获取批量大小
        sparse_embeddings = torch.empty((bs, 0, self.embed_dim), device=self._get_device())  # 初始化稀疏嵌入张量，形状为 (bs, 0, self.embed_dim)，并放置在模型所在的设备上
        if points is not None:# 如果存在点提示
            coords, labels = points  # 解包点提示，得到点坐标和标签
            point_embeddings = self._embed_points(coords, labels, pad=(boxes is None))# 调用 _embed_points 方法对这些点进行嵌入编码，根据是否存在边界框决定是否进行填充
            sparse_embeddings = torch.cat([sparse_embeddings, point_embeddings], dim=1)# 将点的嵌入结果沿着第二个维度（即嵌入数量维度）拼接到稀疏嵌入张量中
        ...
        if boxes_origin is not None:# 如果存在边界框提示
            ...
            sparse_embeddings = torch.cat([sparse_embeddings, box_embeddings_origin], dim=1) #将边界框的嵌入结果沿着第二个维度拼接到稀疏嵌入张量中
        ...
        if rboxes_origin is not None:# 如果存在边界框提示
            ...
            sparse_embeddings = torch.cat([sparse_embeddings, rbox_embeddings_origin], dim=1) #将边界框的嵌入结果沿着第二个维度拼接到稀疏嵌入张量中
